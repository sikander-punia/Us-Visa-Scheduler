...
def reschedule(date):
    time = get_time(date)
    send_notification("Reschedule", f"Fetched {date} {time} for rescheduling")
...
def get_time(date):
    ...
    info_logger(LOG_FILE_NAME, f"Got date {date} and time {time} for rescheduling")
...
def get_available_date(dates):
    ...
    for d in dates:
        ...
        if is_in_period(date, PSD, PED):
            info_logger(LOG_FILE_NAME, f"date {date} is in period {PSD} and {PED}")
            return date
    ...
    while 1:
        ...
        try:
            msg = (
                "\n\n\n"
                + "-" * 60
                + f"\n Request count: {Req_count}, Log time: {datetime.now().today()}\n"
            )
            print(msg)
            info_logger(LOG_FILE_NAME, msg)
            dates = get_date()
            while dates is None:
                print("dates are none, fetching again")
                info_logger(LOG_FILE_NAME, "dates are none, fetching again")
                dates = get_date()
            if not dates:
                msg = f"List is empty, probably banned. Sleep for {BAN_COOLDOWN_TIME} hours"
                print(msg)
                info_logger(LOG_FILE_NAME, msg)
                ...
                if Req_count == 1:
                    datesMsg = "Dates fetched checking for available slots"
                    send_notification("dates fetched", datesMsg)
                    info_logger(LOG_FILE_NAME, datesMsg)
                    print(datesMsg)
                msg = ""
                for d in dates:
...
print(msg)
info_logger(LOG_FILE_NAME, msg)
send_notification("Program end", msg)
driver.get(SIGN_OUT_LINK)
driver.stop_client()
driver.quit()
